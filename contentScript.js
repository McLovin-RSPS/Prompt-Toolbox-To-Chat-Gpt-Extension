  ...
  function traverse(node) {
    for (let childNode of node.childNodes) {
      if (childNode.nodeType === Node.TEXT_NODE) {
        lines.push(childNode.nodeValue.trim());
      } else if (childNode.nodeType === Node.ELEMENT_NODE) {
        if (childNode.tagName === "BR" || childNode.tagName === "P") {
          lines.push("\n");
        }
        traverse(childNode); // Recursively traverse child elements
      }
    }
  }
  traverse(element);
  ...
  if (window.location.host === "prompttoolbox.com") {
    ...
    if (cardTitles) {
      cardTitles.forEach((cardTitle) => {
        ...
        let text = extractTextWithNewlines(cardTitle);
        ...
        cardTitle.parentNode.insertBefore(linkButton, cardTitle);
      });
    }
...
window.onload = function () {
  ...
  // Event listener for the "Copy All Text" button
  document.getElementById("copyButton")?.addEventListener("click", function () {
    let textToCopy = document.body.innerText;
    if (textToCopy) {
